...
- Added session-only thinking-effort selection to `/switch` and Alt+P.
- Added current-key OpenRouter spending and limit information to `omp usage`.

### Fixed

- Guided goal interviews use the ask tool after read-only recon and preserve queued image attachments.
- Windows external editors hide the launcher window and report launchers that exit without opening the draft.
- Legacy plugins can load JSON, TOML, and text assets.
