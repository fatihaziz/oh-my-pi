...
This checkout carries ONE unified source-level patch for behavior not yet in upstream. The source of truth is `scripts/omp-unified.patch`, generated from the release worktree. `scripts/omp-reapply-patches.py` maps repository source packages to installed npm packages through `PACKAGE_PREFIXES`, embeds the local stats client, rebuilds `dist/cli.js` transactionally, and verifies the bundle markers. The engine's `UNIFIED_BASE_VERSION` owns the patch base. External sync/config repositories MAY invoke the engine but MUST NOT copy its patch logic.

Behaviors carried by the unified patch (marker IDs, verification-only):

| ID | Behavior | Origin | Upstream plan |
|---|---|---|---|
| S1 | `/switch` and Alt+P show and apply session-only thinking effort | upstream PR [#8029](https://github.com/can1357/oh-my-pi/pull/8029), closed unmerged | retain until upstream provides the behavior |
...
| P13 | Astra declares mandatory reasoning; OMP clamps disabled reasoning to `low` instead of sending unsupported `none` (`packages/catalog/src/compat/rules/classes/openai.kdl`) | upstream PR [#11361](https://github.com/can1357/oh-my-pi/pull/11361), open; upstream change is Codex-scoped | retire when an installed upstream release supplies the mandatory Codex Astra metadata |
| P14 | `ctx.companion` exposes an authoritative final-settle snapshot, one semantic submit, a bounded interrupt, and a validated resolver for an open `ask` dialog, so an external client can observe completion and answer a question through supported APIs (`packages/coding-agent/src/session/companion.ts`) | local only; no PR | retire when upstream exposes a public final-settle subscription and an ask-resolution API |
| P15 | Exploration rewind preserves canonical todo updates and clears through reload | generic harness fix | retire when upstream preserves canonical rewind snapshots |
| P16 | Bounded native source selections avoid generic output re-elision | generic harness fix | retire when upstream preserves bounded source fidelity |
| P17 | Enabled checkpoint/rewind retain typed exposure; device parse errors include schemas | generic harness fix | retire when upstream supplies both contracts |
| P18 | Todo accounting uses semantic effects; repetition steering recognizes unchanged-result cycles | generic harness fix | retire when upstream supplies outcome-aware accounting and steering |
| P19 | Invalid tool calls receive bounded argument-schema feedback; repeated pre-dispatch failures across different calls trigger concise recovery, then stop before another model request | local harness fix for smaller models | retire when upstream provides schema-guided bounded protocol recovery |
...
P6/P7 (guided-goal ask-tool and recon-first interviews) are retired by owner decision. Keep upstream `/guided-goal`; do not carry PR #8187 or fork PR #1 into future release integrations.

The release worktree is `tmp/upstream-unified-<version>` on branch `unified-patch-<version>`, with the version taken from `UNIFIED_BASE_VERSION`. Create a fresh worktree for each upstream release. Follow the managed `omp-fork-upstream-rebase` skill and the engine's module docstring. After changing KDL policies, run `bun --cwd=packages/catalog run gen:compat` and include the generated `src/compat/rules.json` in the patch. Update the vault's `___claude-fatih/env.yml` pin, then run its sync with `--only omp-local,omp-cli-patches --force`.
