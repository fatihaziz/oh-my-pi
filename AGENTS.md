...
| P10 | Optional `OMP_GIT_MAX_CONCURRENCY` FIFO limiter caps concurrent `git` subprocess launches (`utils/git.ts:git`) | fork PR [fatihaziz/oh-my-pi#2](https://github.com/fatihaziz/oh-my-pi/pull/2); upstream [#9936](https://github.com/can1357/oh-my-pi/pull/9936) — closed unmerged by owner 2026-09-03 | DEFERRED at 18.1.4: upstream removed `utils/git.ts` (VCS → `@oh-my-pi/pi-natives/vcs`); re-home pending |
