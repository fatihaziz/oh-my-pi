...
| P13 | Astra declares mandatory reasoning; OMP clamps disabled reasoning to `low` instead of sending unsupported `none` (`packages/catalog/src/compat/rules/classes/openai.kdl`) | local only; upstreamable | retire when upstream supplies mandatory Astra reasoning metadata |
